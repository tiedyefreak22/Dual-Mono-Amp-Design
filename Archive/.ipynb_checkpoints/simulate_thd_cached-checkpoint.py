...
def simulate_thd(triode, Vg_op, pk_pk, R_out, CCS_current, Vp_scan=None, f=1000, cycles=3, sampling_rate=100000):
    ...
    # Cached bracket: use last solution as center of new bracket
    last_vp = 150  # mid supply guess
    ...
    for Vg in Vg_in:
        ...
        found = False
        bracket_width = 10  # volts
        try:
            sol = root_scalar(
                residual,
                bracket=[last_vp - bracket_width, last_vp + bracket_width],
                method='brentq'
            )
            if sol.converged:
                last_vp = sol.root
